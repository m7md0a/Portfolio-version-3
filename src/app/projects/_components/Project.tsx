...
export default function Project({ project }: ProjectProps) {
  ...
  return (
    ...
    >
      <section className="flex flex-col-reverse sm:flex-row bg-gray-100 max-w-[42rem] border border-black/5 rounded-lg overflow-hidden sm:pr-8 relative sm:h-[22rem] hover:bg-gray-200 transition sm:group-odd:pl-8 dark:text-white dark:bg-white/10 dark:hover:bg-white/20">
        <div className="pt-4 pb-7 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%] flex flex-col h-full sm:group-odd:ml-[18rem]">
          ...
          <ul className="flex flex-wrap mt-4 gap-2 sm:mt-auto">
            {project.meta.technologies.map((technology, index) => (
              <li
                className="bg-primary/85 px-3 py-1 text-[0.7rem] uppercase tracking-wider text-white rounded-full dark:text-white/70"
                key={index}
              >
                {technology}
              </li>
        ...
        <Link
          ...
          >
            <Image
              ...
              width={400}
              height={400}
              className="sm:absolute  top-12 -right-20 w-full sm:w-[26rem] rounded-t-lg sm:shadow-2xl
                  transition 
