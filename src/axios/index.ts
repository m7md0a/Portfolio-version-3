...
import { PinnedRepoType, ProfileType } from "@/types";
...
export async function getPinnedProjects(): Promise<PinnedRepoType[] | undefined> {
  try {
    let data;
    await axios.get(`https://gh-pinned-api.vercel.app/api?user=m7md0a`).then((res) => {
      data = res.data;
    });
    return data;
  } catch (error) {
